...
template <> struct fmt::formatter<Amount>: formatter<float> {
    auto format(Amount a, format_context& ctx) const {
        return formatter<float>::format(a.dollars(), ctx);
    }
};
...
namespace sqlite_orm {
    ...
    struct field_printer<Amount> {
        std::string operator()(const Amount& a) const {
            return fmt::to_string(a.base());
        }
    };

    template<>
    struct row_extractor<Amount> {
        Amount extract(const char* row_value) {
            return Amount(std::stoi(row_value));
        }
